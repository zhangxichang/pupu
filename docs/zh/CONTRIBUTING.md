# 贡献指南

感谢你花时间考虑为 Pupu 做贡献！本指南将帮助你了解如何有效地报告问题、提交代码或文档改进。

## 开始之前

- **查看现有议题**：在创建新议题（Issue）或拉取请求（Pull Request）前，请先搜索[议题列表](https://github.com/ZhangXiChang/pupu/issues)，确认你的想法是否已被讨论。
- **开始编码前**：无论是修复你发现的 Bug 还是添加新功能，都**建议先创建一个 Issue 进行讨论**。这有助于确保你的工作符合项目目标，并避免重复工作。如果是解决现有的 Issue 那么你就可以不用创建 Issue

## 开发流程

1.  **Fork仓库**：在GitHub上Fork本仓库到你的账户。
2.  **克隆仓库**：将你Fork后的仓库克隆到本地。
    ```bash
    git clone https://github.com/用户名/pupu
    cd pupu
    ```
3.  **创建分支**：从最新的 `main` 分支创建一个新分支。
    ```bash
    git checkout -b <分支名>
    ```
4.  **进行修改并提交**：在本地进行你的修改。
    - **遵循代码风格**：请遵循项目中现有的代码风格和惯例。
    - **进行测试**：如果你添加了代码，请进行相应的测试。
    - **提交信息**：请撰写清晰、具体的提交信息。
5.  **推送分支**：将你的分支推送到你的Fork仓库。
    ```bash
    git push origin <分支名>
    ```

## 提交Pull Request

1.  前往GitHub上你的Fork仓库，点击 **“Compare & pull request”**。
2.  **填写PR模板**：请清晰地描述你的更改内容、原因以及相关Issue编号（如有）。
3.  **等待审查**：项目维护者会审查你的PR，并可能提出修改意见。这是一个正常的协作过程，请积极参与讨论。

## 代码审查标准

- 代码功能正确且符合设计。
- 变更范围清晰，专注于解决一个问题。
- 进行了适当的测试（如适用）。
- 遵循了项目的编码规范。

---

再次感谢你的贡献！你的工作使这个项目变得更好。
