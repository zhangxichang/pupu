...
pub struct SQLiteApiImpl {
    connection_pool: Arc<Slab<Mutex<rusqlite::Connection>>>,
}
#[taurpc::resolvers]
impl SQLiteApi for SQLiteApiImpl {
        ...
        async {
            eyre::Ok(
                self.connection_pool
                    .insert(Mutex::new(rusqlite::Connection::open(path)?))
                    .get_move()?,
            )
        }
        .await
    ...
    async fn close_db(self, id: usize) {
        self.connection_pool.remove(id);
    }
    ...
    async fn query(
        ...
        async {
            let db = self.connection_pool.get(id);
            let db = db.get()?.lock();
